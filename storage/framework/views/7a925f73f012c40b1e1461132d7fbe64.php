...
<div class="max-w-4xl mx-auto mt-10">
    ...
    <div class="bg-white dark:bg-slate-800 p-8 sm:p-12 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-700">
        <form method="post" action="<?php echo e(route('profile.update')); ?>" enctype="multipart/form-data" class="space-y-8">
            <?php echo csrf_field(); ?>
            ...
            <div class="flex items-center gap-8">
                <div class="w-32 h-32 rounded-full overflow-hidden border-4 border-indigo-50 dark:border-indigo-900/30 shadow-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                    ...
                    <?php else: ?>
                        <span class="text-5xl font-black text-indigo-300 dark:text-indigo-500"><?php echo e(substr(auth()->user()->name, 0, 1)); ?></span>
                    <?php endif; ?>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="<?php echo e(auth()->user()->name); ?>" required class="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-3">E-Posta</label>
                    <input type="email" name="email" value="<?php echo e(auth()->user()->email); ?>" required class="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 dark:text-white">
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\CASPER\Herd\blog-deneme\resources\views/profil-duzenle.blade.php ENDPATH**/ ?>