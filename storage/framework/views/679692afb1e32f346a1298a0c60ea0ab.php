...
<body class="min-h-screen flex flex-col bg-[#f8fafc] text-slate-800 dark:bg-slate-900 dark:text-white transition-colors duration-300">
    ...
    <nav class="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex justify-between items-center">
            ...
            <a href="<?php echo e(route('blog.index')); ?>" class="text-xl sm:text-2xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter">
                BLOGPRO.
            </a>
            ...
            <div class="flex items-center gap-3 sm:gap-6">
                ...
                <?php if(auth()->guard()->check()): ?>
                    <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-2 pl-2 sm:pl-4 border-l border-slate-200 dark:border-slate-700 hover:opacity-80 transition-opacity">
                    ...
                    <a href="<?php echo e(route('dashboard')); ?>" class="hidden md:block text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition">Panelim</a>

                    <a href="<?php echo e(route('blog.create')); ?>" class="bg-indigo-600 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-full text-sm font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 dark:shadow-none flex items-center gap-1">
                        <span class="text-lg leading-none">+</span>
                        <span class="hidden sm:inline">Yazı Yaz</span>
                    </a>

                    <form action="<?php echo e(route('logout')); ?>" method="POST" class="inline flex items-center">
                        <?php echo csrf_field(); ?>
                ...
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="text-sm font-bold text-slate-600 dark:text-slate-300">Giriş Yap</a>
                    <a href="<?php echo e(route('register')); ?>" class="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-2 sm:py-2.5 rounded-full text-sm font-bold">Kayıt Ol</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
