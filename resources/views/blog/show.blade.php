...
<div class="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden mt-6 transition-colors duration-300">
    ...
    <div class="p-8 sm:p-12">
        <div class="flex flex-wrap items-center text-sm text-gray-500 dark:text-slate-400 mb-6 gap-3 transition-colors">
            ...
            <span>•</span>
            <span class="font-semibold text-gray-700 dark:text-slate-300 flex items-center transition-colors">👤 {{ $post->user->name }}</span>
            <span>•</span>
        ...
        <div class="mt-12 border-t border-gray-100 dark:border-slate-800 pt-8 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-colors">
            ...
            @auth
                <button onclick="toggleLike({{ $post->id }})" id="like-btn-{{ $post->id }}"
                        class="flex items-center gap-3 px-6 py-3 rounded-2xl transition-all shadow-sm active:scale-95 group border {{ $post->isLikedBy(auth()->user()) ? 'bg-rose-50 dark:bg-rose-900/30 border-rose-100 dark:border-rose-800 text-rose-600 dark:text-rose-400' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:bg-rose-50 dark:hover:bg-rose-900/30 hover:border-rose-100 dark:hover:border-rose-800 hover:text-rose-600 dark:hover:text-rose-400' }}">
                    <span id="like-icon-{{ $post->id }}" class="text-2xl transition-transform group-hover:scale-110">
                        {{ $post->isLikedBy(auth()->user()) ? '❤️' : '🤍' }}
                    </span>
                    <div class="flex flex-col items-start leading-none">
                        <span class="text-[10px] font-bold uppercase tracking-widest">BEĞEN</span>
                        <span id="like-count-{{ $post->id }}" class="text-sm font-extrabold">{{ $post->likes()->count() }}</span>
                    </div>
                </button>
            @else
                <a href="{{ route('login') }}" class="flex items-center gap-3 bg-gray-50 dark:bg-slate-800/50 text-gray-400 dark:text-slate-500 px-6 py-3 rounded-2xl border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-800 transition">
        ...
        <div class="mt-16 border-t border-gray-100 dark:border-slate-800 pt-10 transition-colors">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-8 flex items-center transition-colors">
                💬 Yorumlar <span class="ml-3 bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 text-sm px-3 py-1 rounded-full transition-colors">{{ $post->comments->count() }}</span>
            </h3>
                ...
                @forelse($post->comments as $comment)
                    <div class="flex space-x-4 p-6 bg-white dark:bg-slate-800 rounded-2xl border border-gray-50 dark:border-slate-700 shadow-sm hover:shadow-md dark:hover:shadow-[0_10px_30px_-15px_rgba(0,0,0,0.5)] transition">
                        <div class="flex-shrink-0">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($comment->user_name) }}&background=random&color=fff"
                                 class="w-12 h-12 rounded-full shadow-inner" alt="{{ $comment->user_name }}">
                        </div>
                        <div class="flex-grow">
                            <div class="flex justify-between items-center mb-2">
                                <h5 class="font-bold text-gray-900 dark:text-white transition-colors">{{ $comment->user_name }}</h5>
                                <span class="text-xs text-gray-400 dark:text-slate-500 font-medium italic transition-colors">{{ $comment->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-gray-600 dark:text-slate-300 leading-relaxed transition-colors">{{ $comment->content }}</p>
                        </div>
                    </div>
                @empty
        ...
        <div class="mt-12 border-t border-gray-100 dark:border-slate-800 pt-8 flex justify-between items-center transition-colors">
            <a href="{{ route('blog.index') }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium transition flex items-center group">
