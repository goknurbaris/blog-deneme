...
<div class="max-w-6xl mx-auto">
    ...
    <a href="{{ route('blog.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors mb-6 group">
    ...
    <div class="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium transition-colors">Hoş geldin, <span class="text-indigo-600 dark:text-indigo-400">{{ auth()->user()->name }}</span>! İşte istatistiklerin.</p>
        </div>
        <a href="{{ route('blog.create') }}" class="inline-flex items-center justify-center bg-indigo-600 text-white px-6 py-3 rounded-2xl text-sm font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 dark:shadow-none">
            + Yeni Yazı Yaz
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
        <div class="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-300 group">
            ...
            <div class="flex items-end justify-between">
                <div class="text-5xl font-black text-indigo-600 dark:text-indigo-400 transition-colors">{{ $toplamYazi }}</div>
        ...
        <div class="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-300 group">
            ...
            <div class="flex items-end justify-between">
                <div class="text-5xl font-black text-violet-600 dark:text-violet-400 transition-colors">{{ $toplamYazar }}</div>
        ...
        <div class="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-300 group">
            ...
            <div class="flex items-end justify-between">
                <div class="text-5xl font-black text-rose-600 dark:text-rose-400 transition-colors">{{ $toplamBegeni }}</div>
                <div class="text-3xl opacity-20 group-hover:scale-110 transition-transform duration-300">❤️</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden transition-colors duration-300">
        @forelse($benimYazilarim as $yazi)
            <div class="p-6 border-b border-slate-100 dark:border-slate-700 last:border-0 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 transition-all duration-300 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                ...
                <div class="flex-1">
                    <a href="{{ route('blog.show', $yazi->slug) }}" class="text-lg font-bold text-slate-800 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors block mb-1">
                        {{ $yazi->title }}
                    </a>
                    <div class="flex flex-wrap items-center text-xs text-slate-500 dark:text-slate-400 font-medium gap-3 transition-colors">
                        <span class="bg-slate-100 dark:bg-slate-900 px-2 py-1 rounded-md">{{ $yazi->created_at->format('d.m.Y') }}</span>

                        <span class="text-indigo-500 dark:text-indigo-400 font-bold flex items-center gap-1">
                            ⏱️ {{ $yazi->okuma_suresi }} dk okuma
                        </span>

                        <span>❤️ {{ $yazi->likes()->count() }} Beğeni</span>
                        <span>💬 {{ $yazi->comments()->count() }} Yorum</span>
                    </div>
                </div>

                <div class="flex items-center gap-3 w-full sm:w-auto">
                    <a href="{{ route('blog.edit', $yazi->id) }}" class="flex-1 sm:flex-none text-center px-5 py-2.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">
                        Düzenle
                    </a>

                    <form action="{{ route('blog.destroy', $yazi->id) }}" method="POST" class="flex-1 sm:flex-none" onsubmit="return confirm('Bu yazıyı silmek istediğine emin misin? (Bu işlem geri alınamaz)');">
                        @csrf
                        @method('delete')
        ...
        @empty
            <div class="p-16 text-center">
                ...
                <a href="{{ route('blog.create') }}" class="inline-flex items-center text-indigo-600 dark:text-indigo-400 font-black text-xs uppercase tracking-widest hover:text-indigo-700 transition-colors">
                    İlk Yazını Yaz →
                </a>
            </div>
        @endforelse
